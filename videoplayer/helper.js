...
SceneManager.prototype.initVPlayerSession = function( title, url, titleHeader, subs, thumb, drm) {
	debug("Init session vplayer");
	if(!GLOBALS.vplayer) initVideoPlayer();
	var drmOn = (drm && drm.length > 0 ? true: false);
	...
	if (drmOn) {
		GLOBALS.scenemgr.focus.drm = "playready";
		GLOBALS.scenemgr.focus.la_url = drm;
		GLOBALS.vplayer.currentItem.drm = "playready";
		GLOBALS.vplayer.currentItem.la_url = GLOBALS.scenemgr.focus.la_url;
	}
	...
	prepareVideoStart(GLOBALS.vplayer.currentItem);
	...
	if(ENABLE_MIDDLE || ENABLE_PREROLL){
		...
		if (0) { // tests
			...
			var tim = 15, end = 60*60*3; // add midrolls per 10 mins for max 3 hours
			if (GLOBALS.dev)
				tim=1;

			for(var i = tim; i < end; i+= tim) {
...
function prepareVideoStart(item){
	if (!item)
		return;
	...
	if( profile.version == "mse-eme" ){
		if( item.la_url ){
			GLOBALS.vplayer.setDRM( item.drm, item.la_url );
		...
		if(profile.hbbtv != '1.1' && self.focus.la_url != '' ){
...
function showInfo( msg, timeout, inMs )
{
	...
	try {
		...
		debug( "error in show info function: " + e.message );
	}
}
/*
...
function XMLEscape(sValue, bUseApos, bStripTags) {
	try{
		...
		debug("Error: XMLEscape: " + e.description);
		return "";
	}
}
...
window.onerror = function(message, url, lineNumber) { 
	...
	try{
		debug(message + " url: " + url + " line: " + lineNumber ); // + " " + (new Error()).stack);
	} catch(e){
		debug("Error: onerror: " + e.description);
	}
...
function updateOnlineStatus(event) {
    ...
    debug("Connection state changed to: " + condition);
...
function displayChannelList(){	
	...
	try {
		...
		for (var i=0; i<lst.length; i++) {
			var ch = lst.item(i);
			debug(i+1, ch.name, ch.ccid, ch.onid, ch.sid, ch.tsid);
...
function getChannelData(){
	try{
		...
		*/
		...
		debug( "Chlist length " + chlist.length + " ");
...
function deleteAllCookies(param1) {
	...
	debug("Deleted cookie 'RefappWatched'");
}
...
var getCapabilities = function() {
	try {
		...
		debug("error getting capabilities");
		capabilities = null;
	}
...
function setOIPFActiveDRM__(item){
	debug("[helper setOIPFActiveDRM]");
 ...
 var retval=false;
	...
	try {
		...
		debug(desc + ", "+drmSystemId + ", retval="+retval);
	} catch(ex) {
		debug(drmSystemId + ", retval="+ex.message);
	}
}
...
function setOIPFActiveDRM(item) {
	...
	item.desc="setActiveDRM(playready)";
	debug("[helper setOIPFActiveDRM] item: ");
	debug(item);
	var drmSystemId;
	var desc;
	if(item) {
	...
	try {
		...
		debug(desc + ", "+drmSystemId + ", retval="+ex.message);
	}
}
...
function getDeviceID(sysid, callback) {
	...
	var drmAgent = $("#oipfDrm")[0];
	if(!drmAgent) {
		debug("oipfDrm is null");
		return;
	}
	
	debug("Get deviceId for " + DRM_SYSTEM_ID);
	drmAgent.onDRMMessageResult = function(msgID, resultMsg, resultCode) {
		debug("msgID: " + msgID + ", resultCode: " + resultCode
			+ ", resultMsg: " + resultMsg);
		try {
