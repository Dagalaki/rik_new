...
Search.prototype.init = function (parent, xpos, ypos) {
    this.parent = parent;
    var e = createClassDiv(0, 0, "search");
...
SearchInput.prototype.init = function (parent, xpos, ypos) {
    ...
    this.inputDiv = createClassDiv(10, 9, "input-div");
...
SearchInput.prototype.search = function () {
    var me = this,
        url = "search.php?action=search&keyword=" + this.inputText;
    // url = "http://movie.anixa.tv/movies_db_access.php?action=search&keyword=" + this.inputText;
    //url = "http://movie.anixa.tv/movies_db_access.php?action=search&keyword=DAWN";
    this.req = createHttpRequest(url, function (ret) {
        me.req = null;
        if (!GLOBALS.focusmgr.getObject("search-results")) var e = new SearchResults("search-results");
        else {
        ...
        return true;
        var d = JSON.parse(ret);
        llog("[SearchInput.prototype.search] Data: ", d);
        if (d.length == 0) {
            if (GLOBALS.focusmgr.getObject("search-results")) {
                var e = GLOBALS.focusmgr.getObject("search-results");
                e.elem.style.visibility = "hidden";
...
SearchResults.prototype.init = function (parent, xpos, ypos) {
    this.parent = parent;
    var e = createClassDiv(70, /*186*/ 236, "results-container");
    this.parent.appendChild(e);
    this.baseInit(e);
    this.register();
    this.buttons = [];
...
SearchResults.prototype.load = function (data) {
    ...
    if (d[0]) var elems = JSON.parse(d[0].elems);
    else return true;
    llog("[SearchResults.prototype.load] elems: ", elems);
    /*if(!GLOBALS.focusmgr.getObject("search-results-list")){
        var results = new HorizontalList("search-results-list" , "");
    //	results.hideFocus();
    }else {
        results= GLOBALS.focusmgr.getObject("search-results-list");
        results.parent.removeChild(results.elem);
        results.buttons = [];
    }*/
    var results = new HorizontalList("search-results-list");
    results.inSearch = true;
    results.initMoviesList(this.elem, 0, 0, elems, "");
    //	if(results.items.length ==1 ) GLOBALS.focusmgr.focusObject("search-results-list");
}
...
KeyboardTab.prototype.init = function (parent, xpos, ypos) {
    this.parent = parent;
    var e = createClassDiv(0, 0, "keyboard-tab-container");
    this.parent.appendChild(e);
    this.baseInit(e);
    this.register();
    this.buttons = [];
...
KeyboardTab.prototype.handleKeyPress = function (keyCode) {
    ...
    switch (keyCode) {
        ...
        case VK_RIGHT:
            ...
            if (this.focusedId == 5 || this.focusedId == 11 || this.focusedId == 17 || this.focusedId == 23 || this.focusedId == this.buttons.length - 1) {
        ...
        case VK_DOWN:
            ...
            if (this.currentLine > this.numLines) {
                //if((this.focusedId + this.perline) > (this.buttons.length-1)){
                if (GLOBALS.focusmgr.getObject("search-results")) {
...
SearchKeyboard.prototype.init = function (parent, xpos, ypos) {
    ...
    var obj = new KeyboardTab("keyboard_symbols");
    obj.init(this.elem, 15, 0);
    obj.hide();
    this.buttons[1] = obj;
...
var keyboard_buttons = [{
        "image_on": "img/search/search-white.png",
        "image_off": "img/search/search-black.png",
        "class_name": "search-btn"
    },
    {
        "image_on": "img/search/delete-white.png",
        "image_off": "img/search/delete-black.png",
        "class_name": "delete-all-btn"
    },
    {
        "image_on": "img/search/backspace-white.png",
        "image_off": "img/search/backspace-black.png",
        "class_name": "delete-char-btn"
    },
    {
        "image_on": "img/search/language-white.png",
        "image_off": "img/search/language-black.png",
        "class_name": "switch-keyboard-btn"
    },
    {
        "image_on": "img/search/numbers-symbols-white.png",
        "image_off": "img/search/numbers-symbols-black.png",
        "class_name": "num-keyboard-btn"
    },
    /*{"image_on": "img/search/space-white.png", "image_off": "img/search/space-black.png", "class_name": "space-keyboard-btn"}
     */
];
...
KeyboardButtons.prototype.init = function (parent, xpos, ypos) {
    this.parent = parent;
    var e = createClassDiv(320, 0, "keyboard-btn-container");
    this.parent.appendChild(e);
    this.baseInit(e);
    this.register();
    this.buttons = [];
...
KeyboardButtons.prototype.setFocused = function (otherobj, focus) {
    ...
    for (var i = 0; i < this.buttons.length; i++) {
        if (focus) {
            if (i == this.focusedId) this.buttons[i].getElementsByTagName("img")[0].src = keyboard_buttons[i].image_on;
            else this.buttons[i].getElementsByTagName("img")[0].src = keyboard_buttons[i].image_off;
        } else {
            this.buttons[i].getElementsByTagName("img")[0].src = keyboard_buttons[i].image_off;
        }
    }
}
...
KeyboardButtons.prototype.handleKeyPress = function (keyCode) {
    ...
    switch (keyCode) {
        ...
        case VK_UP:
            this.focusedId--;
            if (this.focusedId < 0) GLOBALS.focusmgr.focusObject("search-input");
            this.setFocused(this.idnam, true);
            break;
        case VK_ENTER:
            var obj = GLOBALS.focusmgr.getObject("search-input");
            if (this.focusedId == 0) {
                ...
                var obj = GLOBALS.focusmgr.getObject("search-keyboard");
                if (obj.ktab_idnam == "keyboard_symbols") obj.ktab_idnam = "keyboard_en";
                var newtab = "keyboard_en";
                obj.ktab_idnam = newtab;
                obj.switchKeyboard(newtab);
            } else if (this.focusedId == 4) {
                var obj = GLOBALS.focusmgr.getObject("search-keyboard");
                obj.ktab_idnam = "keyboard_symbols";
                obj.switchKeyboard("keyboard_symbols");
            }
            break;
        default:
            break;
