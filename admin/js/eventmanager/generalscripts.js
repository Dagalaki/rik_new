...
function clearSelectedFingerprints() {
	var sfp = document.getElementById("chosen-fingerprints");
	while (sfp.firstChild) {
		sfp.removeChild(sfp.firstChild);
	}
}

function fillSelFPList(responsetext) {
	var ulField = document.getElementById("chosen-fingerprints");
	clearSelectedFingerprints();
	var jsonArray = JSON.parse(responsetext);
	if (jsonArray["fp"].length > 0) {
		for (var i = 0; i < jsonArray["fp"].length; i++) {
			addSelFP(jsonArray["fp"][i],"load_node");
		}
	}
	//console.log(jsonArray["fp"]);
}

function removeFPEntry(triggerelement) {
	var triggerparent = triggerelement.parentNode;
	for (var i = 0; i < triggerparent.childNodes.length; i++) {
		if (triggerparent.childNodes[i].innerHTML == triggerelement.innerHTML) {
			switch(triggerelement.className) {
				case "new_node":
					triggerparent.removeChild(triggerparent.childNodes[i]);
					break;
				case "load_node":
					triggerelement.className = "remove_node";
					break;
				case "remove_node":
					triggerelement.className = "load_node";
					break;
				default:
...
function addSelFP(innerString, mark) {
	var ulField = document.getElementById("chosen-fingerprints");
	var tempLi = document.createElement("li");
	if (mark != "") {
		tempLi.className = mark;
	}
	tempLi.onclick = function() {removeFPEntry(this);};
	tempLi.innerHTML = innerString;
	ulField.appendChild(tempLi);
}

function selectFingerprint() {
	var seFi = document.getElementById("select-fingerprints");
	var selOption = seFi.childNodes[seFi.selectedIndex];
	addSelFP(selOption.innerHTML,"new_node");
}

function closeAddMotive() {
	var tmp = document.getElementById("pAddMotive").parentNode;
	tmp.removeChild(document.getElementById("pAddMotive"));
}

function motiveOk() {
	hasNewMotiveDialogue = false;
	var tempText = document.getElementById("txtAddMotive").value;
	var seUM = document.getElementById("select-usermotiv");
	//buildMotiveSettings();
	arrayToOptions(seUM, [tempText], "0");
}

function motiveAbort() {
	hasNewMotiveDialogue = false;
	buildMotiveSettings();
	console.log("Test Abort");
}

function reloadMotiveSettings() {
	var tempIter = document.getElementById("select-usermotiv").selectedIndex;
	buildMotiveSettings();
	document.getElementById("select-usermotiv").selectedIndex = tempIter;
}

var hasNewMotiveDialogue = false;

function addMotive() {
	if (!hasNewMotiveDialogue) {
		var parentText = document.getElementById("select-usermotiv").parentNode.parentNode;
		var pContainer = document.createElement("p");
		pContainer.id="pAddMotive";
		pContainer.style.clear = "both";
		var modTxtMotiv = document.createElement("input");
		modTxtMotiv.id = "txtAddMotive";
		modTxtMotiv.type = "text";
		modTxtMotiv.style.width = "46%";
		modTxtMotiv.style.marginTop = "1em";
		modTxtMotiv.style.marginRight = "1.45em";
		modTxtMotiv.style.float = "left";
		var modBtnOk = document.createElement("input");
		modBtnOk.type = "button";
		modBtnOk.value = "OK";
		modBtnOk.onclick = function() {motiveOk();};
		modBtnOk.className = "button";
		modBtnOk.id = "buMotiveOk";
		modBtnOk.name = "buMotiveOk";
		modBtnOk.style.width = "23%";
		modBtnOk.style.marginTop = "0.899em";
		modBtnOk.style.marginRight = "2%";
		modBtnOk.style.float = "left";
		
		var modBtnAbort = document.createElement("input");
		modBtnAbort.type = "button";
		modBtnAbort.value = "Abbrechen";
		modBtnAbort.onclick = function() {motiveAbort();};
		modBtnAbort.className = "button";
		modBtnAbort.id = "buMotiveAbort";
		modBtnAbort.name = "buMotiveAbort";
		modBtnAbort.style.width = "23%";
		modBtnAbort.style.marginTop = "0.899em";
		modBtnAbort.style.float = "left";
		
		pContainer.appendChild(modTxtMotiv);
		pContainer.appendChild(modBtnOk);
		pContainer.appendChild(modBtnAbort);
		parentText.appendChild(pContainer);
		// Zeige ein Texteingabefeld
		// Zeige zwei Buttons
		//console.log(parentText.tagName);
		hasNewMotiveDialogue = true;
	}
}

function removeMotive() {
	//console.log("TestRem");
	var motive = document.getElementById("select-usermotiv").value;
	emptySend.open("get", "triggermanager/?removemotive=" + motive + "&checkremove=true");
	emptySend.send();
}
...
function buildMotiveSettings() {
	...
	var divMotiv = document.createElement("div");
	divMotiv.className = "columns";
	var pMotiv = document.createElement("p");
	pMotiv.className = "colx2-left";
	var pFingerprints = document.createElement("p");
	var pSelFingerprints = document.createElement("p");
	//pSelFingerprints.className = "colx2-left";
	var pMotivButtons = document.createElement("p");
	pMotivButtons.className = "colx2-right columns";
	
	var lbMotivButtons = document.createElement("label");
	lbMotivButtons.style.width = "100%";
	lbMotivButtons.htmlFor = "buAddMotive";
	lbMotivButtons.innerHTML = "Motiveinstellungen";
	
	var buAddMotive = document.createElement("input");
	buAddMotive.value = "Neues Motiv";
	buAddMotive.onclick = function() {addMotive();};
	buAddMotive.type = "button";
	buAddMotive.className = "button colx2-left";
	buAddMotive.id = "buAddMotive";
	buAddMotive.name = "buAddMotive";
	buAddMotive.style.width = "48%";
	var buRemMotive = document.createElement("input");
	buRemMotive.value = "Motiv löschen";
	buRemMotive.onclick = function() {removeMotive();};
	buRemMotive.type = "button";
	buRemMotive.className = "button colx2-right";
	buRemMotive.id = "buRemMotive";
	buRemMotive.name = "buRemMotive";
	...
	seMotiv.onchange = function() {getMotives();};
	arrayToOptions(seMotiv, distinctArray(userMotives), "0");
	...
	seFingerprints.onchange = function() {selectFingerprint();};
	//seTime.type = "text";
	...
	var lbSelFingerprints = document.createElement("label");
	lbSelFingerprints.htmlFor = "chosen-fingerprints-container";
	lbSelFingerprints.className = "margin-top";
	lbSelFingerprints.innerHTML = "Ausgewählte Fingerprints";
	
	var divSelFingerprints = document.createElement("div");
	divSelFingerprints.id = "chosen-fingerprints-container";
	divSelFingerprints.style.height = "20em";
	//divSelFingerprints.className = "block-content";
	
	var liSelFingerprints = document.createElement("ul");
	liSelFingerprints.id = "chosen-fingerprints";
	liSelFingerprints.className = "mini-blocks-list";
	...
	pMotivButtons.appendChild(lbMotivButtons);
	pMotivButtons.appendChild(buAddMotive);
	pMotivButtons.appendChild(buRemMotive);
	divMotiv.appendChild(pMotiv);
	divMotiv.appendChild(pMotivButtons);
	divBoxes.appendChild(divMotiv);
	
	pFingerprints.appendChild(lbFingerprints);
	pFingerprints.appendChild(seFingerprints);
	divDown.appendChild(pFingerprints);
	
	divSelFingerprints.appendChild(liSelFingerprints);
	pSelFingerprints.appendChild(lbSelFingerprints);
	pSelFingerprints.appendChild(divSelFingerprints);
	divDown.appendChild(pSelFingerprints);
...
checkfps.onload = function() {
	try {
		if (this.responseText != "") {
			...
			fillSelFPList(this.responseText);
		}
	} catch (err) {
	}
};

var emptySend = new XMLHttpRequest();

emptySend.onload = function() {
...
function arrayToOptions(parentelement, optionArray, campaignId) {
	...
	//console.log("Array To Options Test");
}
...
/**
  ...
  */
function actualizeFingerprints() {
	var fingerprintNodes = document.getElementById("chosen-fingerprints").childNodes;
	var selMotive = document.getElementById("select-usermotiv").value;
	
	var tempArray = new Array();
	if (fingerprintNodes.length < 1) {
		alert("Keine Fingerprints zum Motiv ausgewählt!");
		return false;
	}
	for (var i = 0; i < fingerprintNodes.length; i++) {
		tempArray[i] = fingerprintNodes[i].innerHTML;
	}
	var tempJSON = JSON.stringify(tempArray);
	var res = confirm("Wirklich erstellen für '" + selMotive + "'?");
	if (res) {
		sendJSON.open("get", "triggermanager/?fpn=" + tempJSON + "&fpnjs=" + selMotive + "&setFingerprints=true");
		sendJSON.send();
	}
...
function buildTriggerSettings(triggerelement, isGoogle) {
	...
	arrayToOptions(seMotiv, userMotives, triggerParent.id);
	//arrayToOptions(seMotiv, ["-----"], "0");
