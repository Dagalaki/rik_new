...
$hits = []; $infos = [];
$debug = false; $readonly = false; $skipInfos = false; $doRep = false;
...
foreach ($hits as $s) {
	...
	if ($ins) {
		...
		if (!$readonly) {
			...
			if ($doRep) $dupmysqli->query("INSERT INTO durations_n SET id = '". $id ."', ts = ". $ts .", ip = ". $ip .", end = ". $ts .", smart_id = ". $smartId ." ON DUPLICATE KEY UPDATE end = ". $ts .", smart_id = ". $smartId);
		}
		if ($mysqli->error)
...
foreach ($updates as $s) {
	...
	if (!$readonly) {
		...
		if ($doRep) $dupmysqli->query("UPDATE durations_n SET end = ". $s['end'] ." WHERE id = '". $s['id'] ."' AND ts = ". $s['ts']);
	}
	if ($mysqli->error)
		die($mysqli->error);
	$k++;
